...
This project renders a video as painted watercolor animation with p5.brush. Frames are rendered offline in headless Chrome, so speed matters less than quality, within a budget. The shot list and the user's brief are in STORYBOARD.md; timing, audio and chapter list are in `song.js`. Unless the brief says otherwise: **cute, cartoony, fun colors, lively animation, something happening in every shot, be brave and ambitious.**
...
```js
// src/ch/c03_takeoff.js (a chapter listed in song.js as 'c03_takeoff')
(() => {
  ...
  CAST.basilisk = (x, y, s, t, o = {}) => { ... }; // only for guests a later chapter reuses
})();
```
...
- **If `song.js` has lyrics, the karaoke bar covers the bottom band (about y 975–1070) whenever a lyric is showing.** Keep faces and key action above about y 960.
- The paper texture is already under every frame, and a paper grain and vignette are multiplied over the top.
- The timeline adds these automatically: karaoke (from `SONG.lyrics`) and brush-wipe chapter breaks at the `SONG.wipes` times.
...
- **Timing:** `bpOf(t)` gives the beat position (`BEAT` = 60 / `SONG.bpm` seconds, a bar is 4 beats). Also:
...
**The Researcher**: `researcher(x, y, s, o)`, a ready-made human character. A small human: lab coat, round glasses, scribbly hair. About 13.2s tall; `s ≈ 0.75u` makes their head level with Clawd's eyes.
...
```
node render.mjs --sheet=10.1,10.8,11.6,12.4,13.1,13.8 --cols=3 --w=640 --out=out/check/c03_a.jpg
node render.mjs --stills=11,14.5 --out=out/check/c03_full
```
